        ...
        return (
            <div style={{marginBottom:12}}>
                <div style={{ display: "flex",marginBottom:5,marginTop:8 }}>
                    ...
                    {category.type === "income" ?
                        <Text fw={700} fz="md">primit de la: {<Badge color="green">{category.name}</Badge>}</Text>  : <Text fw={700} fz="md">cheltuit pe: {<Badge  color="red">{category.name}</Badge>}</Text>}
    ...
    const amountCol = (amount, type) => {
        return (
            <div style={{marginBottom:12}}>
                ...
                <Text fw={700} fz="md" style={{marginBottom:12,color: '#26AB35'}}>{"+ Ron. " + amount.toLocaleString("ro-RO")}</Text> : <Text fw={700} fz="md" style={{marginBottom:12}}>{"- Ron. " + amount.toLocaleString("ro-RO")}</Text>}
            </div>
        )
    }
