...
export default function AccountList() {
  ...
  const rows = accountList.map((element) => (
    <tr key={element.accountId}>
      ...
      <td><Text fw={700}>{`Ron. ${element.totalIncome ? element.totalIncome.toLocaleString("ro-RO") : 0}`}</Text></td>
      <td><Text fw={700}>{`Ron. ${element.totalExpenses ? element.totalExpenses.toLocaleString("ro-RO") : 0}`}</Text></td>
      <td><Text fw={700} style={{ color: "#26AB35" }}>{`Ron. ${element.currentBalance ? element.currentBalance.toLocaleString("ro-RO") : 0}`}</Text></td>
