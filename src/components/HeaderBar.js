                    ...
                    {props.isLandingPage ?
                        ...
                        <Group>
                            <Menu radius={"md"} openDelay={100} shadow="md" width={220}>
                                <Menu.Target>
                                    <UnstyledButton style={{ height: rem(42) }} radius={"md"} variant={"default"}>
                                        <Group>
                                            ...
                                            {!props.isMobile &&
                                                <div style={{ flex: 1 }}>
                                                    <Text size="sm" fw={700}>{currentUser.firstName}</Text>
                                                    <Text c={"dimmed"} size="xs">{currentUser.email.length > 16 ? `${currentUser.email.slice(0, 16)}...` : currentUser.email}</Text>
                                                </div>
                                            }
                                ...
                                <Menu.Dropdown>
                                    <Menu.Item transitionProps={{ transition: 'slide-down', duration: 150 }} onClick={() => { handleSetting() }} icon={<ProfileIcon style={{ height: 16, width: 16 }} />}><Text size={"sm"}>Profilul meu</Text></Menu.Item>
                                    <Menu.Item transitionProps={{ transition: 'slide-down', duration: 150 }} onClick={() => setDisplayConfirmLogout(true)} color="red" icon={<LogoutIcon style={{ height: 16, width: 16 }} />}><Text size={"sm"}>Delogare</Text></Menu.Item>
                                </Menu.Dropdown>
                            </Menu>
                        </Group>
                    }
                ...
                >
                    <Container>
                        ...
                        <Group position="right" mt="md">
                            <Button radius="md" onClick={() => { setDisplayConfirmLogout(false) }} variant={"default"}>Nu, înapoi</Button>
                            <Button radius="md" onClick={() => { handleLogout() }} color='red'>Da, deloghează-mă</Button>
                        </Group>
                    </Container>
